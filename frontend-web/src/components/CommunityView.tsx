        ...
        <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-700">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-8 py-2 text-sm bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-650 rounded-lg text-slate-800 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all"
            />
            {searchQuery && (
              <button
          ...
          {!loadingMembers && filteredMembers.map(user => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors text-left border-b border-slate-50 dark:border-slate-700/50 ${
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-0.5">
                    <span className={`text-sm truncate ${isActive ? 'font-semibold text-teal-700 dark:text-teal-400' : 'font-medium text-slate-800 dark:text-slate-200'}`}>
                      {user.first_name} {user.last_name}
                    </span>
                    {lastMsg && (
                      <span className="text-xs text-slate-400 dark:text-slate-500 flex-shrink-0 ml-1">{formatTime(lastMsg.created_at)}</span>
                    )}
                  </div>
